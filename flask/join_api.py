...
def join_info():
    ...
    if pass_id:
        ...
        if item:
            ...
            if lau_user:
                obj_time_distin = db.session.query(
                    OrdObject.startOrd_time).filter(
                    OrdObject.itemId == item.item_id).distinct().order_by(
                    OrdObject.startOrd_time).all()
                ...
                for data in obj_data:
                    ...
                    data['start_time'] = [
                        ...
                        data['start_time'].minute]  # 修改开始时间格式
                    # print(date) # 返回日期列表
                ...
                datadict = {
                    ...
                    "end_time": json.dumps(
                        item.end_time,
                        cls=DateEncoder),
                    "contacter": item.contacter if item.contacter else lau_user.nickname,
    ...
    def decorated(data):
        ...
        if not (objIds and ord_price):
            ...
            item = db.session.query(Item).filter(
                OrdObject.obj_id.in_(objIds)).filter(
                OrdObject.item).first()
            # 创建订单
            neword = Order(
                ord_itemId=item.item_id,
                ord_usId=ord_usId,
                ord_price=ord_price)
            ...
            try:
                ...
                for objId in objIds:
                    ...
                    if orded_obj.residue > 0:
                        ...
                        # 创建被预定的订单详情
                        newordinfo = Orderinfo(
                            ordNum=neword.ord_num, objId=objId, queue_num=queue_num)
