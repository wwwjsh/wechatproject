...
def get_user(js_code, app_id=appid, secret=secret):
    # 执行测试 所用的数据
    # return {'openid': 'bbbUI0egBJY1zhBYw2KhdUfwVJJE', 'session_key':
    # 'tiihtNczf5v6AKRyjwEUhQ==', 'errcode': 0}
...
class Token():
    ...
    def checkbytoken(self, func):  # 通过路由请求，验证登录状态
        @wraps(func)
        def decorated(data):
            data = data if isinstance(data, dict) else data.get_json()
            print(data)
            ...
            if data and data.get('token', 0):
                ...
                return jsonify({"errNum": -1,
                                'errMsg': "token inexistence form Error"})
        return decorated
