...
def get_items():
    itemslist = db.session.query(Item).filter(Item.logic_del != 0,
                                              ...
                                              Item.page_show != 0)
    if itemslist:
        doing_items = [{'name': item.item_name, 'pass_id': item.pass_id, 'type': item.item_type,
                        'text_info':item.text_info, 'img_info': item.img_info}
                        for item in itemslist.filter(Item.start_time <= datetime.datetime.now()).order_by(Item.start_time).all() ]
        willbe_items = [{'name': item.item_name, 'pass_id': item.pass_id, 'type': item.item_type,
                         'text_info': item.text_info, 'img_info': item.img_info}
                        for item in itemslist.filter(Item.start_time > datetime.datetime.now()).order_by(Item.start_time).all()]
        db.session.close()
