...
class Login_test(unittest.TestCase):
    ...
    def test_update_address(self):
        ...
        response = app.test_client().post('/user/update', json={'token': self.token, 'address': "广tm", 'phone':''})
    ...
    def test_update_phone(self):
        '''测试用户更新电话号码'''
        response = app.test_client().post('/user/update', json={'token': self.token,'address': "",
                                                                'phone': '88888'})
        json_data = response.data
        json_dict = json.loads(json_data)
        print('用户更新电话信息 api')
    ...
    def test_update_phoneandadress(self):
        ...
        response = app.test_client().post('/user/update', json={'token': self.token,
                                                                'address': '变成上海',
                                                                'phone': '12222'})
        json_dict = response.get_json()
    ...
    def test_updateuserinfo(self):
        '''修改用户的头像和昵称，获取私密信息'''
        response = app.test_client().post('/userinfo',
...
class launch_test(unittest.TestCase):
    ...
    def test_launchactivaty(self):
        ...
        data = {'token': self.token, "item_name":'中心湖公园玩耍', "item_type": '1',# 1排队类型
                "contacts":'qq:11111555', "start_time": ['2019-05-05 14:00','2019-05-06 15:00'], #两个时间段分别的开始时间
                "end_time": ['2019-05-05 17:00', '2019-05-06 18:00'], "item_address": "中心湖公园",
                "text_info": '大家记得来啊！', "ord_objects":
...
class join_test(unittest.TestCase):
    ...
    def test_join_info(self):
        '''测试是否能获取指定项目的预定信息 '''
        response = app.test_client().get('/joinInfo?wd=gy56k', data={'wd': 'gy56k'}
                                          )
        json_dict = response.get_json()
        print(json_dict)
        self.assertIn("errNum", json_dict, '数据错误缺少参数errNum')
    ...
    def test_joinitem1(self):
        '''测试预定可预定对象，情况： 都有剩余位置 '''
        response = app.test_client().post('/joinAcitvity', json={'token': self.token, 'objId': [57, 58]})
    ...
    def test_joinitem2(self):
        ...
        response = app.test_client().post('/joinAcitvity', json={'token': self.token, 'objId': [105, 106]})

        json_dict = response.get_json()
        print(json_dict)
        self.assertIn("errNum", json_dict, '数据错误缺少参数errNum')
        self.assertEqual(json_dict['errNum'], -1, '状态码错误')
        self.assertEqual(json_dict["errMsg"], 'ObjresidueError.', '有对象被成功预定了 错误的情况被通过')

    def test_joinitem3(self):
        '''测试预定可预定对象，情况： 其中一个没有剩余位置 '''
        response = app.test_client().post('/joinAcitvity', json={'token': self.token, 'objId': [104, 103]})
...
class test_iteminfo(unittest.TestCase):
    ...
    def test_mylauchitemdetail(self):
        '''测试获取自己的项目详情信息'''
        response = app.test_client().post('/iteminfo/Mylaunch/detail', json={'token': self.token, 'wd': 'gba21'})
    ...
    def test_mylauchitemdetail2(self):
        ...
        response = app.test_client().post('/iteminfo/Mylaunch/detail', json={'token': self.token, 'wd': 'gba421'})
    ...
    def test_myjoineditemdetali(self):
        '''测试获取自己的加入项目 '''
        response = app.test_client().post('/iteminfo/Myjoined/detail', json={'token': self.token, 'wd': 'pysnr'})
    ...
    def test_myjoineditemdetali2(self):
        ...
        response = app.test_client().post('/iteminfo/Myjoined/detail', json={'token': self.token, 'wd': 'gba421'})
