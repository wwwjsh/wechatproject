        ...
        if pass_id:
            ...
            if item:
                ...
                ord_objectlist = [{'obj_id': ord_object.obj_id,'obj_num': ord_object.obj_num,
                                   ...
                                   'startOrd_time': datetime_to_list(ord_object.startOrd_time), 'ordable_sum': ord_object.ordable_sum, 'residue': ord_object.residue
...
def myjoined_itemdetail():
    ...
    def decorated(data):
        pass_id = data.get('wd', 0)
        if pass_id:
            myitem = db.session.query(Item).filter(Item.pass_id == pass_id, Item.logic_del == 1).first()
            if myitem:
                lauser = db.session.query(User).filter(User.openid == data['token'].get_openid()).first()
                # 查询订单 查看加入动作 是否被取消 存放到项目相关的信息字段
                orders_cancel_del = db.session.query(Order.cancel_del).filter(Order.ord_itemId == myitem.item_id,
                ...
                myorded_objects = db.session.query(OrdObject, Orderinfo.queue_num, Order.cancel_del).filter(OrdObject.obj_id.in_
                                                      (db.session.query(Orderinfo.objId).filter(Orderinfo.ordNum.in_
                                                                                                         (db.session.query(Order.ord_num).filter(Order.ord_usId == data['token'].get_openid())
                                                                                                          ))), OrdObject.item == myitem, Orderinfo.objId == OrdObject.obj_id, Order.ord_num == Orderinfo.ordNum).all()

                #本项目中我预定了的对象
                ord_objectlist = [{'obj_id': myorded_object[0].obj_id,'obj_num': myorded_object[0].obj_num,
                                   'obj_name': myorded_object[0].obj_name, 'minOrd_time': myorded_object[0].minOrd_time,
                                   'startOrd_time':  datetime_to_list(myorded_object[0].startOrd_time), 'residue': myorded_object[0].residue,
                                   'logic_del': myorded_object[0].logic_del, 'queue_num': myorded_object[1], 'cancel_status': myorded_object[2],
...
def alter_launchinfo():
    ...
    def decorated(data):
        ...
        item = db.session.query(Item).filter(Item.pass_id == pass_id, Item.lau_usId == openid).first()
        if item:
            if page_show != -1:
                ...
                # 取消动作 不能重新发起需要重新填写表单
                if logic_del == 0:
                    objs = db.session.query(OrdObject).filter(OrdObject.itemId == item.item_id)
                    for obj in objs:
                        obj.logic_del = 0
                        db.session.add(obj)
            else:
            ...
            db.session.commit()
            db.session.close()
            info = {"errNum": 0, "errMsg": 'success', 'data': 'page_show_changed'}
            return jsonify(info)
        else:
            db.session.close()
            info = {"errNum": -1, "errMsg": "itemError."}
...
def alter_joininfo():
    ...
    def decorated(data):
        ...
        pass_id = data.get('wd', 0)
        # 找到要取消加入的项目
        item = db.session.query(Item).filter(Item.pass_id == pass_id, Item.logic_del == 1).first()
        if item:
            # 找到要取消 加入项目 和 该用户相关的对应的 订单
            alter_orders = db.session.query(Order).filter(Order.ord_itemId == item.item_id, Order.ord_usId == openid)
            if alter_orders:
                # 修改相关字段 cancel_del
