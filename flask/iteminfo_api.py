...
def mylaunch_itemlist():
    ...
    def decorated(data):
        ...
        if itemlist:
            ...
            info = {'item_todict': item_todict, "errNum": 0, "errMsg": "success"}
            return jsonify(info)
        else:
...
def mylaunch_itemdetail():
    ...
    def decorated(data):
        ...
        if pass_id:
            item = db.session.query(Item).filter(Item.pass_id == pass_id, User.openid == data['token'].get_openid()).first()
            if item:
...
def myjoined_itemlist():
    ...
    def decorated(data):
        ...
        if itemlist:
            ...
            item_todict = [{'item_name': item.item_name, 'pass_id': item.pass_id, 'item_type': item.item_type,
                            'overed': (item.end_time<datetime.datetime.now()), 'start_time': json.dumps(item.start_time, cls=DateEncoder),
                            'end_time': json.dumps(item.end_time, cls=DateEncoder), 'logic_del': item.logic_del} for item in itemlist]
